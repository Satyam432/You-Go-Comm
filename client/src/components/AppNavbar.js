...
const AppNavbar = () => {
	...
	const incompleteProfile = useSelector(
		(state) => state.auth.incompleteDetails
	);
		...
		<React.Fragment>
			{incompleteProfile && (
				<div style={{ background: 'white' }}>
					...
					<Link to='/account-details'>Complete Details</Link>
				</div>
			)}
			...
			<Navbar className='sticky-top' id='AppNavbarSmall' light>
				...
				<Collapse
					...
					<Nav navbar className='ml-0'>
						...
						<NavItem active className='nav1'>
							{isAuthenticated ? (
								<a href={`${HOST}/api/auth/logout/`} className='nav-link pl-4'>
									Logout
								</a>
							) : (
								<a href={`${HOST}/api/auth/google/`} className='nav-link pl-4'>
									Login
								</a>
							)}
						</NavItem>
